...
```{r}
normality.test(modele)
```

## Test Portmanteau (corrélations des résidus)

Les 3 premières p-value ne peuvent être calculées à cause de la valeur des degrés de liberté.
...
La statistique de Portmanteau est $Q_h = T \sum_{j=1}^h tr(\hat{C}_j^T \hat{C}_0^{-1} \hat{C}_j \hat{C}_0^{-1})$. Pour qu'elle existe, il faut donc vérifier que la matrice $\hat{C}_0$ est inversible pour que la statistique puisse être définie. Les matrices $\hat{C}_i$ s'écrivent $\hat{C}_i = \frac{1}{T} \sum_{t=i+1}^T \hat{u}_t \hat{u}_{t-i}^T$, donc $\hat{C}_0$ s'écrit $\hat{C}_0 = \frac{1}{T} \sum_{t=1}^T \hat{u}_t \hat{u}_t^T$. Nous allons donc vérifier qu'elle existe pour le modèle complet que nous avons mis en place.

```{r}
for(i in 1:)
residuals(modele)*residuals(modele)
```
...
##Récupération de la valeur manquante du PIB trimestriel
**A placer après la modélisation ARIMA du PIB.**
Le PIB posséde une valeur en moins par rapport aux autres variables dans l'ensemble des données trimestrielles. Ceci nous impose de négliger la dernière valeur de toutes les autres séries pour que toutes les variables soient étudiées sur la même étendue temporelle. Pour éviter ce problème, nous décidons d'estimer la variable du PIB pour le 2e trimestre de 2017. Afin de faire cela, nous allons utiliser la valeur estimer par le modèle SARIMA correspondant à la variable PIB.
...
ARIMAPIBTrimTest <- forecast(ARIMAPIBTrimTrain, h = 6)
...
new.value <- ARIMAPIBTrimTest$mean[6]
ts(c(trim$PIB[1:109], new.value), start = 1990, end = c(2017, 2), frequency=4)
```
