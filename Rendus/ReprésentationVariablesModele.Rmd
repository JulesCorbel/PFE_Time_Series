...
corrplot(cor(annuelle[1:28,-1]), method = "number", type="lower", p.mat=cor.mtest(annuelle[1:28,-1], 0.95)[[1]], insig="pch", col=colorRampPalette(c("blue", "light blue", "red"))(50))
corrplot(cor(trim[1:109,-1]), method = "number", type="lower", p.mat=cor.mtest(trim[1:109,-1], 0.95)[[1]], insig="pch", col=colorRampPalette(c("blue", "light blue", "red"))(50))
...
```
En rpendant un risque de première espèce de 5%, le test KPSS et le test ADF nous indiquent que la série différenciée est bien stationnaire.
...
```{r}
  SMIC <- ts(annuelle$SMIC, start = 1990, end = c(2017, 4))
...
```{r}
  TCHO <- ts(annuelle$TCHO, start = 1990, end = c(2017, 4))
  plot(TCHO, main="Evolution du taux de chômage trimestriel")
  acf(TCHO, main="Auto-corrélation du taux de chômage trimestriel")
  pacf(TCHO, "Autocorrélation partielle du taux de chômage trimestriel")
```
