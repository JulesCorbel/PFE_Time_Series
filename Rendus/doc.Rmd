...
Pendant la deuxième partie du projet, nous nous sommes donc attachés à utiliser d'autres méthodes de modélisation. La plus grande partie de nos travaux portent sur les modèles de type vectoriels, qui permettent donc de prédire plusieurs séries temporelles simultanément
...
```{r, fig.width=3.5, fig.height=3, fig.cap="\\label{fig10} Décomposition de la masse salariale", echo=F}
par(cex.main=0.8)
...
```{r, fig.width=4, fig.height=3.5, fig.cap="\\label{fig13} PIB trimestriel stationnarisé", echo=F}
...
EQM(MSETest, ordre4$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest)
plot(MSETest)
lines(window(ordre4$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest, start=2016, end=c(2017,2)), col="red")
```
...
```{r}
ordre3 <- forecast(VAR(cbind(MSESta, PIBSta, SMICSta, TCHOFSta), p=3, type="const"))
EQM(MSETest, ordre3$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest)
plot(MSETest)
lines(window(ordre3$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest, start=2016, end=c(2017,2)), col="red")
```
...
```{r}
ordre2 <- forecast(VAR(cbind(MSESta, PIBSta, SMICSta, TCHOFSta), p=2, type="const"))
EQM(MSETest, ordre2$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest)
plot(MSETest)
lines(window(ordre2$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest, start=2016, end=c(2017,2)), col="red")
```
...
lines(window(VARSMICSta$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest, start=2016, end=c(2017,2)), col = "red")
...
```{r}
EQM(MSETest, window(VARSMICSta$forecast$MSESta$mean*MSETrendTest$mean*MSESeasonalTest, start=2016, end=c(2017,2)))
```
