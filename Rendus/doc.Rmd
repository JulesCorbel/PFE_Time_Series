...
```{r, fig.width=5, fig.height=4, fig.cap="\\label{fig25} Evolution de la p-value en fonction du lag", echo=F}
a1 <- c()
for(i in 1:18){
...
Afin de mettre en place une modélisation VAR, nous devons dans un premier temps nous intéresser à l'ordre p du modèle VAR. Dans le package **MTS**, la fonction utilisée est VARorder, qui comme VARselect utilise les critères d'AIC, BIC et HQ afin de déterminer l'ordre du processus. Toutefois, le critère FPE n'est pas présent, ce qui nous conforte dans notre idée qu'il n'est pas très utile à notre étude. 
...
Les résultats associés aux modèles d'ordre 3 et 2 sont eux présents en \ref{Annexe4} et en \ref{Annexe5}. Comme pour le package **vars**, il nous faut maintenant vérifier les hypothèses du modèle.
